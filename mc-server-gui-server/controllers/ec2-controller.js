...
const connectToEc2Ssh = (socket) => {
	client
		.on("ready", () => {
			...
			isConnected = true;
			socket.emit("connection_output", { message: "Connected to EC2", isConnected: true });
		})
		.on("error", (err) => {
			console.error("Error connecting to EC2: ", err);
			socket.emit("connection_output", { message: "Error connecting to EC2", isConnected: false });
		})
		.on("end", () => {
			console.log("Connection ended");
			isConnected = false;
			socket.emit("connection_output", { message: "EC2 Connection Ended", isConnected: false });
		})
		.on("close", () => {
			console.log("Connection closed");
			isConnected = false;
			socket.emit("connection_output", { message: "EC2 Connection Closed", isConnected: false });
		})
...
const disconnectFromEc2Ssh = (socket) => {
	client.end();
	isConnected = false;
	socket.emit("connection_output", { message: "Disconnected from EC2", isConnected: false });
};

const getLogs = (socket) => {
	client.exec("tail -f /opt/minecraft/server/logs/latest.log", (err, stream) => {
		if (err) console.log(err);
		stream
			.on("close", (code, signal) => {
				console.log("Stream :: close :: code: " + code + ", signal: " + signal);
...
const stopMcServer = (socket) => {
	...
	client.exec("killall screen", (err, stream) => {
		...
		stream
			...
			.on("data", (data) => {
				console.log("STDOUT: " + data);
				socket.emit("start_stop_output", { message: "Server Stopped", data: data, isRunning: false });
			})
			.stderr.on("data", (data) => {
				console.log("STDERR: " + data);
			});
...
export { connectToEc2Ssh, disconnectFromEc2Ssh, cdToServerDir, startMcServer, stopMcServer };
